...
export interface RoleViewHook {
  ...
  canViewField: (field: 'zones_write' | 'raw_events' | 'volunteer_tasks' | 'incident_dispatch' | 'audit_logs' | 'sustainability_details') => boolean;
}

// RBAC Permissions Mapping
const FIELD_PERMISSIONS: Record<UserRole, Set<string>> = {
  organizer: new Set(['zones_write', 'raw_events', 'volunteer_tasks', 'incident_dispatch', 'audit_logs', 'sustainability_details']),
  security: new Set(['zones_write', 'raw_events', 'incident_dispatch', 'audit_logs', 'sustainability_details']),
...
export const useRoleView = (): RoleViewHook => {
  ...
  const canViewField = (field: 'zones_write' | 'raw_events' | 'volunteer_tasks' | 'incident_dispatch' | 'audit_logs' | 'sustainability_details'): boolean => {
    if (!currentUser) return false;
